...
class DBManager: NSObject {
  ...
  // MARK: - Update App Activate Value
  func updateAppAdd(packageName: String, isAdd: Bool) -> Bool {
      let updateQuery = "UPDATE App SET isAdd = ? WHERE packageName = ?;"
      var updateStatement: OpaquePointer? = nil

      if sqlite3_prepare_v2(db, updateQuery, -1, &updateStatement, nil) == SQLITE_OK {
          let SQLITE_TRANSIENT = unsafeBitCast(-1, to: sqlite3_destructor_type.self)

          sqlite3_bind_int(updateStatement, 1, isAdd ? 1 : 0) // Bind isActive value (1 for true, 0 for false)
          sqlite3_bind_text(updateStatement, 2, packageName, -1, SQLITE_TRANSIENT) // Bind packageName
